...
// Processa pagamento direto via API /v1/payments (usado pelo Checkout Bricks)
// body inclui: transaction_amount, description, payment_method_id, payer, token (cartão), etc.
async function createPayment(body, idempotencyKey) {
  if (!TOKEN) return { error: 'MP_ACCESS_TOKEN não configurado' };
  ...
  try {
    const res = await fetch('https://api.mercadopago.com/v1/payments', {
      ...
      headers: {
        'Authorization': 'Bearer ' + TOKEN,
        'Content-Type': 'application/json',
        'X-Idempotency-Key': idempotencyKey || (Date.now() + '-' + Math.random().toString(36).slice(2))
      },
      body: JSON.stringify(body),
    ...
    const json = await res.json().catch(() => ({}));
    if (res.ok && json.id) {
      console.log('[mp] pagamento criado ID', json.id, 'status:', json.status);
      return { ok: true, payment: json };
    }
    console.error('[mp] createPayment falhou', res.status, JSON.stringify(json).slice(0,500));
    return { error: json.message || ('HTTP ' + res.status), details: json };
  } catch (e) {
    console.error('[mp] createPayment erro:', e.message);
    return { error: e.message };
  }
}

module.exports = { createPreference, getPayment, mapStatus, createPayment, PUBLIC_KEY };
